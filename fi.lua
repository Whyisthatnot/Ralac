...
local function boostFPS()
    for _, obj in ipairs(workspace:GetDescendants()) do
        -- BasePart (gạch, block, mesh)
        if obj:IsA("BasePart") then
            -- Xoá màu (cho nó 1 màu đơn, hoặc trong suốt)
            obj.Color = Color3.fromRGB(128, 128, 128) -- màu xám nhạt
            obj.Material = Enum.Material.SmoothPlastic
            obj.Reflectance = 0
            -- Nếu muốn tàng hình hết thì:
            -- obj.Transparency = 1
        end
        ...
        -- Xoá highlight (nhiều game hay spam highlight lag)
        if obj:IsA("Highlight") then
            obj:Destroy()
        end

        -- Xoá SurfaceAppearance (roblox mesh PBR)
        if obj:IsA("SurfaceAppearance") then
            obj:Destroy()
        end

        -- Xoá SpecialMesh/Union detail nặng
...
-- Nếu muốn auto áp dụng cho object mới tạo ra:
workspace.DescendantAdded:Connect(function(obj)
    ...
    pcall(function()
        ...
        elseif obj:IsA("ParticleEmitter") or obj:IsA("Trail") or obj:IsA("Beam") then
            obj.Enabled = false
        elseif obj:IsA("Highlight") or obj:IsA("SurfaceAppearance") then
            obj:Destroy()
